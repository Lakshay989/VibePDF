...
### A3. `[x]` Recents (last 20, clearable, persisted)
- **Spec:** P1-VIEW-012
...
### B1. `[x]` Real document actor (replace stub)
...
### B2. `[x]` Encrypted-PDF password prompt
- **Spec:** P1-VIEW-003
...
### B3. `[x]` Document actor: render-page-to-bitmap message
...
### C1. `[x]` Virtual-scrolling page list
...
### C2. `[x]` Zoom + fit modes, persisted per document
- **Spec:** P1-VIEW-006
...
### D1. `[x]` Thumbnails sidebar with lazy generation
- **Spec:** P1-VIEW-008
...
### E1. `[x]` Multi-document tab persistence
- **Spec:** P1-VIEW-011
...
### E2. `[x]` Render-failure log scaffold
- **Spec:** P1-VIEW-004 (the "match or document" reading)
- **Touches (new):** `tests/render-failures.md`, `tests/integration/render_compare.rs`
- **What:** A harness that renders one fixture per row against an expected PNG and writes any divergence into `tests/render-failures.md`. Phase 1's interpretation of P1-VIEW-004 is "match a mainstream reader OR documented here."
...
### E4. `[x]` Phase 1 acceptance fixture generator
...
### E5. `[~]` E2E harness (tauri-driver + WebdriverIO) — harness landed, CI-pending
- **Spec:** infrastructure for every UI step's automated verification
- **Touches (new):** `tests/e2e/wdio.conf.ts`, `tests/e2e/specs/smoke.e2e.ts`, `tests/e2e/tsconfig.json`, `tests/e2e/README.md`, `.github/workflows/e2e.yml`
- **What:** One test that launches the built app with hello.pdf as a CLI argument (the file picker is a native dialog, not webview-automatable) and asserts page 1's canvas renders. Lays the rails for every later step to add an `e2e/` test.
- **Tooling note:** the step originally said Playwright, but Playwright can't drive a Tauri webview; switched to **WebdriverIO + tauri-driver** (the supported Tauri 2 E2E stack). `docs/03`/`docs/04` updated.
- **Acceptance:** `npm run test:e2e` green **on Linux CI** (`tauri-driver` has no macOS support, so it can't run on the dev machine).
- **Status:** harness + CI workflow committed and locally validated as far as possible (TS typechecks, YAML valid, deps resolve). **NOT yet verified green** — the first `e2e.yml` run on CI is the real acceptance; flip to `[x]` once it passes (likely needs a fixup pass, written blind from macOS).
