...
async fn measure_persists_through_save() {
    ...
    let state = handle
        .add_measure(0, "distance".into(), vec![[100.0, 700.0], [300.0, 700.0]], "#1f6feb".into(), "4 m".into(), 1.0, 1.5)
        .await
        .expect("distance");
    assert!(state.can_undo, "a measurement must be undoable");
    handle
        .add_measure(0, "area".into(), vec![[100.0, 600.0], [220.0, 600.0], [160.0, 520.0]], "#1e8449".into(), "0.5 m²".into(), 1.0, 1.5)
        .await
        .expect("area");
...
async fn measure_undo_removes_it() {
    ...
    handle
        .add_measure(0, "distance".into(), vec![[100.0, 700.0], [300.0, 700.0]], "#000000".into(), "4 m".into(), 1.0, 1.5)
        .await
        .expect("add");
...
async fn measure_rejects_bad_kind() {
    ...
    let handle = DocumentActorHandle::spawn(None, id, fixture("hello.pdf"), None).expect("spawn");
    let err = handle
        .add_measure(0, "volume".into(), vec![[0.0, 0.0], [10.0, 10.0]], "#000000".into(), "x".into(), 1.0, 1.0)
        .await;
...
async fn measure_writes_verification_artifact() {
    ...
    handle
        .add_measure(0, "distance".into(), vec![[90.0, 700.0], [320.0, 700.0]], "#1f6feb".into(), "4.60 m".into(), 1.0, 1.5)
        .await
        .expect("distance");
    handle
        .add_measure(0, "area".into(), vec![[100.0, 560.0], [300.0, 560.0], [320.0, 460.0], [120.0, 440.0]], "#1e8449".into(), "3.2 m²".into(), 1.0, 1.5)
        .await
        .expect("area");
    handle.save(Some(out.clone())).await.expect("save");
