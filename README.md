...
Roughly 745 frontend tests across 127 files, and 80 Rust integration suites
...
Bundled third-party components keep their own licences, all permissive.
[`THIRD-PARTY-LICENSES.md`](THIRD-PARTY-LICENSES.md) is the full inventory —
generated by `npm run licenses` from the resolved dependency trees, not written
by hand, because a licence list that goes stale makes a false claim about what
the binary contains. The same script refuses to pass if a shipped component
carries a licence outside a reviewed permissive set.

[`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md) explains the components that
carry a real redistribution obligation and how each is met, and
[`NOTICE`](NOTICE) holds the Apache-2.0 attributions. The application shows the
same inventory under **Licences** in the toolbar — a file beside the executable
satisfies the letter of the obligation and is read by nobody.
